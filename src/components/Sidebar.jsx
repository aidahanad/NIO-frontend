...
export default function Sidebar() {
  const { projects, activeProjectId, activeView, activeChatSession } =
    useSyncExternalStore(store.subscribe, store.getSnapshot)

  const [hoveredId, setHoveredId] = useState(null)

  const isChatArea = activeView === 'chat'
  const isProjectArea = activeView === 'project' || activeView === 'new-project'
  const isWelcome = activeView === 'welcome'

  const showDiscussions = isWelcome || isChatArea
  const showProjects = isWelcome || isProjectArea
  ...
  function renameProject(id, name) {
    store.dispatch({ type: 'RENAME_PROJECT', payload: { id, name } })
  }

  function renameChat(title) {
    store.dispatch({ type: 'RENAME_CHAT', payload: title })
  }
  ...
  return (
    ...
    >
      ...
      {/* Discussions section: only on welcome or chat */}
      {showDiscussions && (
        <>
          <p style={sectionTitleStyle}>Discussions</p>

          {!isChatArea && (
            <SidebarOutlineBtn label="+ Nouvelle" onClick={startChat} />
          )}

          {isChatArea && !activeChatSession && (
            <SidebarOutlineBtn label="+ Nouvelle discussion" onClick={startChat} />
          )}
      ...
      {/* Projects section: only on welcome or project/study */}
      {showProjects && (
        <>
          <p style={sectionTitleStyle}>Projets</p>
          ...
          {activeView === 'new-project' && (
            <SidebarActiveBtn label="Nouveau projet" />
          )}
...
function SidebarActiveBtn({ label }) {
  return (
    <div
      style={{
        width: '100%',
